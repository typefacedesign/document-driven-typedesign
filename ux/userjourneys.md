...
* Examples from DC:
    * Shaping. This means long columns of words, one Font per col, and a couple of correct 'control' Fonts preloaded in cols. The Compare modes could be useful here. Example: [conjuncts.pdf](/ux/samples/conjuncts.pdf)
* Examples from PI:
	* Headlines
	* Text
	* adhesion
...
CM: Initially thought you were referring to the Test section for this but I guess this applies to all areas of the application. I've started a very early look into this (so tiny you might miss it!) with a tooltip explaining that a user can add more fonts. This appears once the user has added their first font. I'll try to work more of this into the Wireframes during the next set of days.
...
CM: I strongly believe we need to separate the 2 modes to reduce the complexity of this app, but again without knowing the specifics of how the tests are meant to work I can't know for certain if we can make them work together. We could potentially let users split the test screen into 2 columns and they can choose which 2 fonts sit alongside each other, just not sure at the moment how this would look. I think a good way to test this relatively quickly would be to have a question that we can use for the purposes of creating these wireframes – can you provide one you feel would be good to use?
...
** DC: Part of the 'beta' idea is that users want to see the fonts evolve over time... so being able to 'play' a series of instances of the same family over time. Like Etherpad, playing the document being typed. This could be part of the compare mode? Highlight changes as an animation....**
